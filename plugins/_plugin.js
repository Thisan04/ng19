...
let msg = Config.LANG == 'SI' || Config.LANG == 'AZ' ? '*❤ SL-ASIATA අනුමත කරන ලද ප්ලගීනයකි..* ✅' : '*This Plugin is Officially Approved!* ✅'
...
Asiata.addcommand({pattern: 'plug ?(.*)', fromMe: true, desc: Lang.INSTALL_DESC, warn: Lang.WARN, dontAddCommandList: false}, (async (message, match) => {
    ...
    if (response.statusCode == 200) {
        ...
        if (find.includes(plugin_name)) {
            ...
            if (!match[1].includes('Thisan05') && DEG.level > 99) {
                await message.client.sendMessage(message.jid,LANG.limit + DEG.level + '_', MessageType.text)
                fs.unlinkSync('/root/plugins/' + plugin_name + '.js')
            }
            else if (!match[1].includes('Thisan05') && DEG.level < 100) {
...
Asiata.addcommand({pattern: 'myplugin$', fromMe: true, dontAddCommandList: false, desc: Lang.PLUGIN_DESC}, (async (message, match) => {
    ...
    if (plugins.length < 1) {
        ...
        plugins.map(
            (plugin) => {
                let vf = plugin.dataValues.url.includes('Thisan05') ? msg : unmsg
...
Asiata.addcommand({pattern: 'unplug(?: |$)(.*)', fromMe: true, dontAddCommandList: false, desc: Lang.REMOVE_DESC}, (async (message, match) => {
